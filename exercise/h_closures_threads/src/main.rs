...
fn main() {
    ...
    let child_1_rx = rx_main.clone(); 
    let child_2_rx = rx_main.clone(); 

    let child_1 = thread::spawn(||{
        for msg in child_1_rx {
            println!("Child 1 received {}", msg)
        }
    });

    let child_2 = thread::spawn(|| {
        for msg in child_2_rx {
            println!("Child 2 received {}", msg)
        }
    });

    for msg in vec![1, 2, 3, 4, 5, 6, 7, 8, 9] {
